...
class HexTile : MonoBehaviour
{
    public Vector3 worldPos;
    public Vector3Int cubeCoordinate;
    public TileType tileType;
    public void GetTileType(TileBase tile , TileGenerator tileGenerator)
    {
        if (tile == null)
        {
            tileType = TileType.Unknown;
        }
        else if (tile == tileGenerator.dirtTile || tile == tileGenerator.dirtRandTree1 || tile == tileGenerator.dirtRandTree2 || 
                 tile == tileGenerator.dirtTileStone || tile == tileGenerator.dirtForestTile1 || tile == tileGenerator.dirtForestTile2 || 
                 tile == tileGenerator.dirtForestStone)
        {
            tileType = TileType.Dirt;
        }
        else if (tile == tileGenerator.stoneTile || tile == tileGenerator.stoneTileTree1 || tile == tileGenerator.stoneTileTree2)
        {
            tileType = TileType.Stone;
        }
        else if (tile == tileGenerator.sandTile)
        {
            tileType = TileType.Sand;
        }
        else if (tile == tileGenerator.desertTile || tile == tileGenerator.desertTileCactus1 || tile == tileGenerator.desertTileCactus2 || 
                 tile == tileGenerator.desertTileDirt)
        {
            tileType = TileType.Desert;
        }
        else if (tile == tileGenerator.grassTile || tile == tileGenerator.grassRandTree1 || tile == tileGenerator.grassRandTree2)
        {
            tileType = TileType.Grass;
        }
        else if (tile == tileGenerator.waterTileWave || tile == tileGenerator.waterTile)
        {
            tileType = TileType.Water;
        }
        else if (tile == tileGenerator.riverTile || tile == tileGenerator.riverBankTile)
        {
            tileType = TileType.River;
        }
        else if (tile == tileGenerator.forestTile || tile == tileGenerator.forestTileStone1 || tile == tileGenerator.forestTileStone2 || 
                 tile == tileGenerator.forestTiledirt || tile == tileGenerator.forestCampSite)
        {
            tileType = TileType.Forest;
        }
        else if (IsHouseTile(tile, tileGenerator))
        {
    ...
    private bool IsHouseTile(TileBase tile, TileGenerator tileGenerator)
    {
        return tileGenerator.grassHouseTiles.Contains(tile) ||
               tileGenerator.dirtHouseTiles.Contains(tile) ||
               tileGenerator.stoneHouseTile.Contains(tile) ||
               tileGenerator.desertHouseTiles.Contains(tile);
    }
...
public class TileGenerator : MonoBehaviour
{
    ...
    public  TileBase[] desertHouseTiles;
    ...
    {   
        ...
        for (int x = 0; x < mapWidth; x++)
        {
            for (int y = 0; y < mapHeight; y++)
            {
                ...
                GameObject myObject = new GameObject($"Hex C{x}, R{y}");
                myObject.transform.position = worldPosition;

                // Add the HexTile component
                HexTile hexTileComponent = myObject.AddComponent<HexTile>();

                // Set properties
                hexTileComponent.worldPos = worldPosition;
                Vector3Int cubeCoord = OffsetToCubeCoordinates(x, y);
                hexTileComponent.cubeCoordinate = cubeCoord;

                // Determine and set the tile type
                hexTileComponent.GetTileType(selectedTile, this);
    ...
    private Vector3Int OffsetToCubeCoordinates(int x, int y)
    {
        int cubeX = x - (y - (y & 1)) / 2;
        int cubeZ = y;
    ...
    void GenerateTowns()
    {
        for (int i = 0; i < numberOfTowns; i++)
        {
            ...
            while (!townPlaced && attempts < maxAttempts)
            {
                ...
                if (baseTile == grassTile || baseTile == dirtTile || baseTile == stoneTile || baseTile == desertTile)
                {
    ...
    bool IsAreaClearForTown(int centerX, int centerY)
    {
        ...
        foreach (Vector2Int townCenter in townCenters)
        {
            ...
            int minDistance = radius + townRadius + 20; 
            ...
            for (int y = centerY - radius; y <= centerY + radius; y++)
            {
                if (IsInBounds(x, y))
                {
                    ...
                    if (dx * dx + dy * dy <= radius * radius)
                    {
                        ...
                        else if (baseTile == desertTile)
                        {
                            houseTile = desertHouseTiles[Random.Range(0, desertHouseTiles.Length)];
                        }
